...
export function PopupSignalement(props: PopupSignalementProps) {
  ...
  return (
    <Modal animationType="slide" visible={modalSignalementVisible} transparent={true}>
      <View style={$containerSignalement}>
        <View style={$containerTitrePopup}>
          <Image
            tintColor={
              signalement.type == "PointInteret" ? colors.palette.vert : colors.palette.rouge
            }
            source={
              signalement.type == "PointInteret"
                ? require("assets/icons/view.png")
                : require("assets/icons/attentionV2.png")
            }
            style={$iconeStyle}
          />
          <View style={{ width: "80%" }}>
            <Text weight="bold" size="xl" style={$titreSignalement}>
              {signalement.nom}
            </Text>
          </View>
          <Image
        ...
        {estEntier ? (
          <View style={$containerImageDesc}>
            <Image
              source={{ uri: `data:image/png;base64,${signalement.image}` }}
              style={$imageStyle}
            />
            <Text
              style={[
        ...
        <View style={$containerBoutons}>
          ...
          <Button
            ...
            tx={
              estEntier
                ? "detailsExcursion.popup.signalement.voirMoins"
                : "detailsExcursion.popup.signalement.voirPlus"
            }
